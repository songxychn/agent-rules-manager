...
Agent Rules Manager 是一个 local-first、开源的桌面应用与 CLI。它面向已经同时使用 Codex、Claude Code、Grok、OpenCode 等工具的人：规则内容只维护一份，并继续使用 VS Code、Cursor、Typora、记事本等熟悉的软件编辑；本工具负责检测目标、预览变更、处理各 Agent 的接入差异，并提供可审计回滚。
...
- Codex、Grok 和 OpenCode 使用指向规范源的符号链接。
- Claude Code 保留原有 `CLAUDE.md` 内容，只插入可识别的 `@AGENTS.md` 托管区块。
- 写入前生成只读计划；发现未托管文件或异常链接时，整次应用被阻止。
...
| Agent | 原生路径 | 接入方式 |
| --- | --- | --- |
| Claude Code | `~/.claude/CLAUDE.md` | 托管 include 区块，保留现有内容 |
| Codex | `~/.codex/AGENTS.md` | 符号链接 |
| Grok | `~/.grok/AGENTS.md` | 符号链接 |
| OpenCode | `~/.config/opencode/AGENTS.md` | 符号链接 |
...
桌面控制台会检测当前系统中可用的常见编辑器，并通过“上次选择 + 下拉切换”的分体按钮打开规范 `AGENTS.md`。Finder / 文件资源管理器用于显示文件，终端目标打开规则库目录；前端只提交固定目标 id，实际路径和启动参数由 Tauri 后端决定。
...
路线图包括规则包、机器/工作场景 profiles、可配置 adapters、导入现有规则和签名发布。实现约束见 [架构说明](docs/ARCHITECTURE.md)，参与方式见 [CONTRIBUTING.md](CONTRIBUTING.md)。
