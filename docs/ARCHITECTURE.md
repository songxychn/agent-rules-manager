...
- `upgrade`: schema v1 Rule Packs or schema v2 multi-file Profiles;
- `ready`: supported single-file Profile schema v3;
...
A Profile is a directory whose name matches `profile.json.id`. Its only rule source is the regular UTF-8 file `AGENTS.md`. Profile metadata contains no configurable source list. Symlink sources are rejected.

Legacy v1/v2 lists are read only during previewed migration. Their files are merged in their original order, with exact source contents retained and separating newlines added. Backups precede changes; unknown files and unsafe paths stop migration.
...
Generation reads the Profile's AGENTS.md. Output starts with a generated warning and identifies that source:
...
- each legacy Profile receives one `AGENTS.md`, merging referenced source contents in legacy render order with separating newlines;
